...
export function getViewsBySaasChartData({
  timezone,
  productId,
  userId,
  interval,
  ...
  const cacheFn = dbCache(getViewsBySaasChartDataInternal, {
    tags: [
      getUserTag(userId, CACHE_TAGS.productViews),
      productId == null
        ? getUserTag(userId, CACHE_TAGS.products)
...
async function getViewsBySaasChartDataInternal({
  timezone,
  productId,
  userId,
  interval,
  ...
  return await db
    .with(productViewSq)
    .select({
      saasName: CountryGroupTable.name,
    ...
    .groupBy(({ saasName }) => [saasName])
    .orderBy(({ saasName }) => saasName);
}
